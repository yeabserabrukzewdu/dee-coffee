    ...
    <footer className="bg-[#181818] text-gray-400">
      <div className="container mx-auto px-6 py-12">
        <div className="grid md:grid-cols-4 gap-8">
          <div>
            <h3 className="font-display text-xl font-bold text-white mb-4">Layodacha</h3>
            <p className="text-sm">{t('footer.tagline')}</p>
          </div>
          <div>
            <h4 className="font-bold text-white mb-4 tracking-wider">{t('footer.shop.title')}</h4>
            <ul>
              <li className="mb-2"><a href="#" className="hover:text-white">{t('footer.shop.all')}</a></li>
              <li className="mb-2"><a href="#" className="hover:text-white">{t('footer.shop.subscriptions')}</a></li>
              <li className="mb-2"><a href="#" className="hover:text-white">{t('footer.shop.gear')}</a></li>
            </ul>
          </div>
          <div>
            <h4 className="font-bold text-white mb-4 tracking-wider">{t('footer.about.title')}</h4>
            <ul>
              <li className="mb-2"><a href="#story" className="hover:text-white">{t('footer.about.story')}</a></li>
              <li className="mb-2"><a href="#" className="hover:text-white">{t('footer.about.contact')}</a></li>
              <li className="mb-2"><a href="#" className="hover:text-white">{t('footer.about.faqs')}</a></li>
            </ul>
          </div>
          <div>
            <h4 className="font-bold text-white mb-4 tracking-wider">{t('footer.follow')}</h4>
            <div className="flex space-x-4">
              <a href="#" aria-label="Facebook">
                <svg className="w-6 h-6 hover:text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v2.385z" /></svg>
              </a>
